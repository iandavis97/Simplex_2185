...
uint MyRigidBody::SAT(MyRigidBody* const a_pOther)
{
	...
	*/
	float ra, rb;
	matrix4 R, AbsR;
	std::vector<float>u;//represent this local axes
	std::vector<float>w;//represent other local axes
	u.push_back(GetCenterLocal().x);
	u.push_back(GetCenterLocal().y);
	u.push_back(GetCenterLocal().z);
	w.push_back(a_pOther->GetCenterLocal().x);
	w.push_back(a_pOther->GetCenterLocal().y);
	w.push_back(a_pOther->GetCenterLocal().z);
	...
	t = vector3(glm::dot(t.x, u[0]), glm::dot(t.y, u[1]), glm::dot(t.z, u[2]));
